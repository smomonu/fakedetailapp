...
const WhatsAppGenerator: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      {/* WhatsApp Web Clone Generator Section */}
      <section id="generator" className="py-12 bg-gray-100">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="mb-8 flex items-center justify-between flex-wrap gap-4">
            <div>
              <h2 className="text-3xl font-bold text-gray-900 mb-2">Interactive Chat Generator</h2>
              <p className="text-gray-600">Create and customize your WhatsApp conversations below</p>
            </div>
          ...
          <div className={`mx-auto bg-gray-200 rounded-lg overflow-hidden shadow-2xl ${isMobilePreview ? 'max-w-[420px]' : 'w-full'}`}>
            <div ref={fullChatRef} className="bg-[#111b21] h-[700px] flex flex-col">
              {/* Top Bar */}
              <div className="bg-[#202c33] h-14 flex items-center justify-between px-4 flex-shrink-0">
                <div className="flex items-center gap-3">
                  <img
                ...
                <div className="flex items-center gap-5">
                  <button className="text-gray-400 hover:text-white" onClick={() => setTopNavPlusPopup(!topNavPlusPopup)}>
                    <Plus className="w-5 h-5" />
                  </button>
                  <button className="text-gray-400 hover:text-white" onClick={() => setTopNavPopup(v => !v)}>
              ...
              <div className="flex flex-1 overflow-hidden">
                {/* Chats List Sidebar */}
                <div className={`${selectedChatId ? 'hidden lg:block' : 'block'} w-full lg:w-[380px] bg-[#111b21] border-r border-gray-800 flex flex-col flex-shrink-0`}>
                  {/* Search Bar */}
                    ...
                    {filterChats(chats.sort((a: any, b: any) => new Date(b.updatedAt || 0).getTime() - new Date(a.updatedAt || 0).getTime())).map((chat: any) => (
                      <div
                        ...
                        onClick={() => {
                          ...
                          setViewChat(true);
                          setChats((prev: any) => prev.map((c: any) => c.id === chat.id ? { ...c, unread: 0 } : c));
                        }}
                        className={`flex items-center gap-3 px-4 py-3 hover:bg-[#202c33] cursor-pointer border-b border-gray-800/50 relative group ${
                      ...
                      >
                        ...
                        <button
                          ...
                          className="absolute right-2 top-1/2 -translate-y-1/2 opacity-0 group-hover:opacity-100 text-gray-400 hover:text-white p-1"
                        >
                ...
                {selectedChatId && selectedChat ? (
                  <div className="flex-1 flex flex-col bg-[#0b141a]">
                    {/* Chat Header */}
                    <div className="bg-[#202c33] h-14 flex items-center justify-between px-4 border-b border-gray-800 flex-shrink-0">
                      <div className="flex items-center gap-3">
                        <button
                          onClick={() => { setSelectedChatId(null); setViewChat(false); }}
                        ...
                        <div>
                          {editingName ? (
                            <input
                              ...
                              className="bg-[#2a3942] text-white text-sm font-medium px-2 py-1 rounded outline-none"
                              autoFocus
                            />
                          ) : (
                            <button
                              onClick={() => {
                                setNameInput(selectedChat.name);
                                setEditingName(true);
                          ...
                          <button
                            onClick={toggleSelectedChatOnline}
                      ...
                      <div className="flex items-center gap-5">
                        ...
                        <button className="text-gray-400 hover:text-white" onClick={() => setShowSettingsPanel(!showSettingsPanel)}>
                          <MoreVertical className="w-5 h-5" />
                        </button>
                      </div>
                    </div>
                    ...
                    <div
                      ...
                      style={{ backgroundImage: `url('/images/fakdetail-app-whatsapp-background.jpg')`, backgroundSize: 'cover', backgroundPosition: 'center' }}
                    >
                      {selectedChat.messages.map((msg: any) => {
                        ...
                        return (
                          <div
                            ...
                            className={`flex ${isMe ? 'justify-end' : 'justify-start'} group relative ${highlightedMessageId === msg.id ? 'animate-pulse' : ''}`}
                          >
                            <div
                              className={`max-w-[70%] rounded-lg px-3 py-2 shadow-sm ${
                                isMe ? 'bg-[#005c4b]' : 'bg-[#202c33]'
                              } ${editingMessageId === msg.id ? 'ring-2 ring-blue-500' : ''}`}
                            >
                              {msg.reply && (
                                <div className="bg-black/20 rounded mb-2 px-2 py-1 text-xs border-l-2 border-green-500">
                                  <div className="text-green-400 font-medium">{msg.reply.sender === 'me' ? 'You' : msg.reply.sender}</div>
                                  <div className="text-gray-300 truncate">{msg.reply.text}</div>
                                </div>
                              )}
                              {msg.type === 'image' && msg.url && (
                                <img src={msg.url} alt="attachment" className="rounded mb-1 max-w-full" />
                              )}
                              ...
                              {msg.type === 'voice' && (
                                <div className="flex items-center gap-2">
                                  ...
                                  <div className="h-1 w-32 bg-gray-600 rounded-full">
                                    <div className="h-full w-1/3 bg-green-400 rounded-full"></div>
                                  </div>
                                  <span className="text-xs text-gray-400">{msg.duration || '0:00'}</span>
                                </div>
                              )}
                              {editingMessageId === msg.id ? (
                                <input
                                  ...
                                  className="bg-[#2a3942] text-white text-sm px-2 py-1 rounded outline-none w-full"
                                  autoFocus
                                />
                              ) : (
                                msg.text && <p className="text-white text-sm whitespace-pre-wrap break-words">{msg.text}</p>
                              )}
                              <div className="flex items-center justify-end gap-1 mt-1">
                                <span className="text-gray-400 text-xs">{msg.time}</span>
                                {isMe && (
                                  <CheckCheck className={`w-4 h-4 ${msg.read ? 'text-blue-400' : msg.delivered ? 'text-gray-400' : 'text-gray-600'}`} />
                                )}
                              </div>
                              {msg.reactions && Object.keys(msg.reactions).length > 0 && (
                                <div className="flex gap-1 mt-1">
                                  {Object.entries(msg.reactions).map(([emoji, count]) => (
                                    <span key={emoji} className="text-xs bg-black/30 px-1 rounded">{emoji} {count as number > 1 ? count : ''}</span>
                                  ))}
                                </div>
                              )}
                            </div>
                    ...
                    <ChatComposer
                      onSend={(text, options) => {
                        if (text.trim()) {
                          sendMessage(text, options?.side);
                        } else if (options?.type && options?.url) {
                          sendMediaMessage({ type: options.type as any, url: options.url, sender: options?.side });
                        }
                      }}
                      replyTo={replyTo}
                  ...
                  <div className="hidden lg:flex flex-1 items-center justify-center bg-[#0b141a]">
                    <div className="text-center">
                      ...
                      <h3 className="text-white text-xl font-medium mb-2">WhatsApp Web</h3>
          ...
          <div className="mt-6 bg-white rounded-lg shadow-md p-6">
            <h3 className="text-lg font-semibold text-gray-900 mb-4">Generator Controls</h3>
            <div className="grid md:grid-cols-2 gap-4">
              ...
              <button
                onClick={() => {
                  if (selectedChatId && fullChatRef.current) {
                    html2canvas(fullChatRef.current, { useCORS: true, scale: 2 }).then(canvas => {
                      canvas.toBlob(blob => {
                        if (blob) {
                          const url = URL.createObjectURL(blob);
                          const a = document.createElement('a');
            ...
            <div className="mt-4">
              <label className="flex items-center gap-2 text-sm text-gray-700">
                <input
                  type="checkbox"
                  checked={autoScrollDuringRecording}
                  onChange={(e) => setAutoScrollDuringRecording(e.target.checked)}
                  className="rounded"
                />
      ...
      {contextMenu.visible && (
        ...
        >
          <button
            onClick={() => {
              const msg = selectedChat?.messages.find((m: any) => m.id === contextMenu.msgId);
              if (msg) {
                setReplyTo(msg);
                closeContextMenu();
              }
          ...
          <button
            onClick={() => {
              setEmojiPicker({ visible: true, msgId: contextMenu.msgId, x: contextMenu.x, y: contextMenu.y });
            }}
            className="w-full text-left px-4 py-2 hover:bg-gray-100 text-sm text-gray-700"
          >
            React
          ...
          <button
            onClick={() => {
              if (confirm('Delete this message?')) {
                setChats((prev: any) => prev.map((c: any) => c.id === selectedChatId ? { ...c, messages: c.messages.filter((m: any) => m.id !== contextMenu.msgId) } : c));
              }
              closeContextMenu();
            }}
      ...
      {/* Chat Menu (for chat list items) */}
      {openChatMenuId && menuCoords && (
        <ChatMenu coords={menuCoords} onClose={closeChatMenu}>
          <button
            onClick={() => {
              setChats((prev: any) => prev.map((c: any) => c.id === openChatMenuId ? { ...c, favourite: !c.favourite } : c));
              closeChatMenu();
            }}
          ...
          <button
            onClick={() => {
              handleDeleteChat(openChatMenuId);
            }}
